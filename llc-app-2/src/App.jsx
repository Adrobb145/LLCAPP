...
export default function App(){
  const [coaches,setCoaches]=useState(COACHES0);
  const [clients,setClients]=useState(CLIENTS0);
  const [programs,setPrograms]=useState(()=>Object.fromEntries(CLIENTS0.map(c=>[c.id,makeProgram(c)])));
  const [logs,setLogs]=useState(()=>Object.fromEntries(CLIENTS0.map(c=>[c.id,seedLogs(c,makeProgram(c))])));
  const [notes,setNotes]=useState({mar:[{date:"Apr 30",author:"Adam",from:"coach",text:"Bar speed on 355 squat looked strong — bumping base next week. Proud of you, Marcus."}]});
  const [meals,setMeals]=useState({});
  const [xp,setXp]=useState({mar:1240,sof:480,jam:260,pri:2100,aly:540,mei:90});
  const [goals,setGoals]=useState({mar:[{id:"g0",text:"Hit a 365 squat by meet day",metric:"squat",target:365,due:"",done:false},{id:"g1",text:"Bodyweight ≥ 195 lb",metric:"bodyweight",target:195,due:"",done:true}]});
  const [checkins,setCheckins]=useState({pri:[{date:"Apr 28",energy:8,sleep:8,stress:3,nutrition:8,mood:8,note:"Feeling strong."}]});
  const [bodylog,setBodylog]=useState({mar:[{date:"w1",w:201},{date:"w2",w:200},{date:"w3",w:199},{date:"w4",w:198}]});
  const [photos,setPhotos]=useState({});
  const [freezes,setFreezes]=useState({mar:2,pri:3,aly:1});
  const [ckday,setCkday]=useState({mar:"Sunday",pri:"Monday"});
  const [attendance,setAttendance]=useState(()=>{const t=new Date();const iso=n=>{const x=new Date(t);x.setDate(t.getDate()-n);return x.toISOString().split("T")[0];};return{mar:[{id:"sa1",date:iso(1),type:"1-on-1",rate:100,attended:true},{id:"sa2",date:iso(4),type:"1-on-1",rate:100,attended:true},{id:"sa3",date:iso(8),type:"1-on-1",rate:100,attended:true},{id:"sa4",date:iso(11),type:"1-on-1",rate:100,attended:true}],sof:[{id:"sa5",date:iso(2),type:"Small Group",rate:150,attended:true},{id:"sa6",date:iso(9),type:"Small Group",rate:150,attended:true}],pri:[{id:"sa7",date:iso(3),type:"PT 45",rate:65,attended:true},{id:"sa8",date:iso(6),type:"PT 45",rate:65,attended:false}]};});
  ...
  const [pillaracts,setPillaracts]=useState(seedPillarActs);
  ...
  const buildSnapshot=()=>{
    const state={};
    clients.forEach(c=>{state[c.id]={program:programs[c.id],logs:logs[c.id]||{},notes:notes[c.id]||[],meals:meals[c.id]||[],goals:goals[c.id]||[],checkins:checkins[c.id]||[],bodylog:bodylog[c.id]||[],photos:photos[c.id]||[],misses:misses[c.id]||[],readiness:readiness[c.id]||[],pillaracts:pillaracts[c.id]||{},attendance:attendance[c.id]||[],formvids:formvids[c.id]||[],xp:xp[c.id]||0,freezes:freezes[c.id]||0,ckday:ckday[c.id]||""};});
  ...
  const loadBackend=async()=>{
    try{
      ...
      const m={programs:{},logs:{},notes:{},meals:{},xp:{},goals:{},checkins:{},bodylog:{},photos:{},freezes:{},ckday:{},attendance:{},misses:{},readiness:{},pillaracts:{},formvids:{}};
      Object.entries(stateById).forEach(([cid,s])=>{if(s.program)m.programs[cid]=s.program;m.logs[cid]=s.logs||{};m.notes[cid]=s.notes||[];m.meals[cid]=s.meals||[];m.xp[cid]=s.xp||0;m.goals[cid]=s.goals||[];m.checkins[cid]=s.checkins||[];m.bodylog[cid]=s.bodylog||[];m.photos[cid]=s.photos||[];m.freezes[cid]=s.freezes||0;if(s.ckday)m.ckday[cid]=s.ckday;m.attendance[cid]=s.attendance||[];m.misses[cid]=s.misses||[];m.readiness[cid]=s.readiness||[];m.pillaracts[cid]=s.pillaracts||{};m.formvids[cid]=s.formvids||[];});
      ...
      setPrograms(m.programs);setLogs(m.logs);setNotes(m.notes);setMeals(m.meals);setXp(m.xp);setGoals(m.goals);setCheckins(m.checkins);setBodylog(m.bodylog);setPhotos(m.photos);setFreezes(m.freezes);setCkday(m.ckday);setAttendance(m.attendance);setMisses(m.misses);setReadiness(m.readiness);if(Object.keys(m.pillaracts).length)setPillaracts(m.pillaracts);setFormvids(m.formvids);
  ...
  return(<div className="app">
    ...
    <div className="main">
      ...
      <div className="content">
        {view==="roster"&&<Roster clients={clients} coaches={coaches} onOpen={openClient} onAddClient={()=>setAddOpen(true)}/>}
