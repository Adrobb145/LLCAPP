...
import { supabase } from "../lib/supabase"; // <-- if your supabase client lives elsewhere, fix THIS line only

export default function Roster({clients,coaches,onOpen,onAddClient,onEdit,onDeleted}){
  const [q,setQ]=useState("");const [status,setStatus]=useState("all");const [sort,setSort]=useState("attention");
  const [toDelete,setToDelete]=useState(null);const [removed,setRemoved]=useState(()=>new Set());
  const visible=clients.filter(c=>!removed.has(c.id));
  const flagged=visible.filter(c=>c.adherence<.8).length;
  const avg=visible.length?Math.round(visible.reduce((a,c)=>a+c.adherence,0)/visible.length*100):0;
  let list=visible.filter(c=>{if(status==="attention"&&c.adherence>=.8)return false;if(status==="ontrack"&&c.adherence<.8)return false;if(q){const s=q.toLowerCase();if(!c.name.toLowerCase().includes(s)&&!c.goal.toLowerCase().includes(s))return false;}return true;});
  ...
  return(<div className="roster">
    <div className="rhead"><div><div className="kick" style={{marginBottom:8}}>Workspace</div><div className="rtitle">Roster</div><div className="rsub">{visible.length} active clients · {coaches.length} coaches</div></div><button className="btn" onClick={onAddClient}>＋ Add Client</button></div>
    <div className="rstats">
      <div className="stat"><div className="stat-l">Active Clients</div><div className="stat-v">{visible.length}</div></div>
    ...
    <div style={{fontSize:11,color:"#807E76",marginBottom:12}}>{list.length} of {visible.length}</div>
    ...
    {toDelete&&<DeleteModal client={toDelete} onClose={()=>setToDelete(null)} onDeleted={id=>{setRemoved(s=>{const n=new Set(s);n.add(id);return n;});setToDelete(null);onDeleted&&onDeleted(id);}}/>}
  </div>);
}

// --- Inline confirm-and-delete modal. Calls your `purge` Edge Function. ---
function DeleteModal({client,onClose,onDeleted}){
  ...
  const run=async()=>{
    if(!ok||busy)return;
    setBusy(true);setErr("");
    const {data,error}=await supabase.functions.invoke("purge",{body:{clientId:client.id}});
    if(error){let m=error.message;try{const j=await error.context.json();if(j?.error)m=j.error;}catch{}setErr(m);setBusy(false);return;}
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"center",justifyContent:"center",padding:16,zIndex:1000}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:380,background:"#fff",borderRadius:16,padding:22,boxShadow:"0 20px 60px rgba(0,0,0,.35)"}}>
        ...
        <div style={{fontSize:13,color:"#555",marginBottom:14,lineHeight:1.5}}>This permanently removes <b>{target}</b>'s programming, logs, and pillar history, and deletes their login from the server. This cannot be undone.</div>
        ...
        <button onClick={run} disabled={!ok||busy} style={{width:"100%",marginTop:12,padding:"12px 16px",borderRadius:10,border:"none",background:"#C01800",color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer",opacity:ok&&!busy?1:.5}}>{busy?"Deleting…":"Permanently delete"}</button>
