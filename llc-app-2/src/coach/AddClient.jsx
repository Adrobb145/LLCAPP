...
export default function AddClient({ onAdd, onClose, backend }) {
  ...
  return (<div style={ov} onClick={busy ? undefined : onClose}>
    <div style={card} onClick={(e) => e.stopPropagation()}>
      ...
      <label style={lbl}>Email — how the athlete signs up & links their account</label><input style={inp} type="email" value={f.email} onChange={set("email")} placeholder="jane@email.com" autoCapitalize="none" autoCorrect="off" spellCheck={false} />
      {backend && (<><label style={lbl}>Temp password — you share this with the athlete for first login</label><input style={inp} value={f.temppw} onChange={set("temppw")} placeholder="6+ characters" autoCapitalize="none" autoCorrect="off" spellCheck={false} /></>)}
