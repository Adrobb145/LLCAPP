...
export default function Sheet({client,program,week,setWeek,logs,onLog,onAddEx,onRemoveEx,notes,onAddNote,coaches,formvids=[],vidUrls={},onReviewVid,photos=[]}){
  ...
  return(<div className="sheetwrap">
    <div className="sheetmain">
      ...
      <div className="meso"><div className="meso-l">Week</div><div className="meso-c">{Array.from({length:client.totalWeeks},(_,i)=>{const w=i+1;const dlw=isDeload(w,tw);const st=w===week?"cur":dlw?"dl":w<client.currentWeek?"done":"up";return(<div key={w} className="mcell" data-s={st} onClick={()=>setWeek(w)}>W{w}{dlw?"·DL":""}</div>);})}</div></div>
