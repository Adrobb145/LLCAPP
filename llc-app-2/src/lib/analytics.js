...
export function athStats(client,program,clogs,meals,checkins,xp){
  ...
  if(program)for(let w=1;w<=weeks;w++)program.days.forEach(d=>{const done=d.ex.every(x=>{const e=clogs[`w${w}|${d.id}|${x.exId}`];return e&&e.sets.length&&e.sets.every(s=>s.done);});if(done)sessions++;});
  const vol=[];if(program)for(let w=1;w<=client.currentWeek;w++){let v=0;program.days.forEach(d=>d.ex.forEach(x=>{const e=clogs[`w${w}|${d.id}|${x.exId}`];if(e)e.sets.forEach(s=>{if(s.done)v+=(Number(s.w)||0)*(Number(s.r)||0);});}));vol.push(Math.round(v/100)/10);}
  const prs={};if(program)MAINLIFTS.forEach(nm=>{const id=ID(nm);if(!id)return;let best=null;for(let w=1;w<=weeks;w++)program.days.forEach(d=>d.ex.forEach(x=>{if(x.exId!==id)return;const e=clogs[`w${w}|${d.id}|${x.exId}`];if(e)e.sets.forEach(s=>{if(s.done){const v=e1rm(Number(s.w)||0,Number(s.r)||0);if(!best||v>best.e)best={e:v,w:Number(s.w)||0,r:Number(s.r)||0};}});}));if(best)prs[nm]=best;});
  const series=MAINLIFTS.map(nm=>{const id=ID(nm);if(!id)return null;const arr=[];for(let w=1;w<=client.currentWeek;w++){let b=0;program&&program.days.forEach(d=>d.ex.forEach(x=>{if(x.exId!==id)return;const e=clogs[`w${w}|${d.id}|${x.exId}`];if(e)e.sets.forEach(s=>{if(s.done){const v=e1rm(Number(s.w)||0,Number(s.r)||0);if(v>b)b=v;}});}));if(b)arr.push(b);}return arr.length>=2?{nm,arr}:null;}).filter(Boolean);
...
export function analyze(client,programs,logs,checkins){
  ...
  let dayN=prog?prog.days.length:0,loggedN=0;
  if(prog)prog.days.forEach(d=>{const done=d.ex.every(x=>{const e=cl[`w${cw}|${d.id}|${x.exId}`];return e&&e.sets.length&&e.sets.every(s=>s.done);});if(done)loggedN++;});
