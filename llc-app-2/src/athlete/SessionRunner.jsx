  ...
  return(<div style={{padding:"14px 14px 90px"}}>
    ...
    {data.map((e,ei)=>(<div key={ei} style={{background:D.card,border:`1px solid ${D.line}`,borderRadius:9,padding:11,marginBottom:8}}>
      ...
      {e.sets.map((s,si)=>(<div key={si} style={{display:"grid",gridTemplateColumns:"22px 1fr 1fr 34px",gap:6,alignItems:"center",marginBottom:5}}>
        ...
        <input type="number" value={s.w} onChange={ev=>upd(ei,si,{w:ev.target.value})} style={{background:D.lift,border:`1px solid ${D.line}`,borderRadius:5,padding:7,color:s.done?D.good:D.ink,fontFamily:"'JetBrains Mono',monospace",textAlign:"center",fontSize:13,outline:"none",width:"100%",minWidth:0,boxSizing:"border-box"}}/>
        <input type="number" value={s.r} onChange={ev=>upd(ei,si,{r:ev.target.value})} style={{background:D.lift,border:`1px solid ${D.line}`,borderRadius:5,padding:7,color:s.done?D.good:D.ink,fontFamily:"'JetBrains Mono',monospace",textAlign:"center",fontSize:13,outline:"none",width:"100%",minWidth:0,boxSizing:"border-box"}}/>
