...
export default function ClientApp({client,program,clogs,meals,notes,goals,bodylog,checkins,xp,coachName,onToggleHabit,onLogMeal,onSaveSession,onXP,onAddCheckin,onSaveGoals,onLogBody,onSendChat,onAIReply,photos,freezes,ckday,onAddPhoto,onUseFreeze,onSetCkday,misses,onLogMiss,onLogReadiness,onLogout,pillaracts={},onSetAct,formvids=[],vidUrls={},onAddFormVid}){
  ...
  return(<div style={{minHeight:"100vh",background:D.bg,color:D.ink,fontFamily:"'Inter Tight',system-ui,sans-serif",maxWidth:480,margin:"0 auto",paddingBottom:74,position:"relative"}}>
    ...
    <div style={{padding:"0 16px"}}>
      {tab==="home"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
        {(()=>{if(!program)return null;const DOWI={Sun:0,Mon:1,Tue:2,Wed:3,Thu:4,Fri:5,Sat:6};const t0=new Date();t0.setHours(0,0,0,0);const monday=new Date(t0);monday.setDate(t0.getDate()-((t0.getDay()+6)%7));const dateFor=dow=>{const d=new Date(monday);d.setDate(monday.getDate()+((DOWI[dow]+6)%7));return d;};const logged=dId=>{const dd=program.days.find(x=>x.id===dId);return dd.ex.every(x=>{const e=clogs[`w${cw}|${dId}|${x.exId}`];return e&&e.sets.length&&e.sets.every(s=>s.done);});};const handled=dId=>misses.some(m=>m.week===cw&&m.dayId===dId);const items=program.days.map(d=>{const dt=dateFor(d.dow);const done=logged(d.id);const st=done?"done":dt.getTime()===t0.getTime()?"today":dt.getTime()<t0.getTime()?"missed":"up";return{d,dt,st,done};});const todayItem=items.find(i=>i.st==="today");const pend=items.find(i=>i.st==="missed"&&!handled(i.d.id));const REASONS=[["Sick","🤒"],["Life / busy","⏰"],["Sore / tired","💢"],["Low drive","😞"],["Traveling","✈️"]];const fmtD=d=>d.toLocaleDateString("en-US",{weekday:"short"});return(<div style={{background:`linear-gradient(135deg,${D.acc}18,${D.card})`,border:`1px solid ${D.line}`,borderRadius:14,padding:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div style={{fontSize:9.5,color:D.acc,fontWeight:700,letterSpacing:".12em",textTransform:"uppercase"}}>Today · {t0.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}{dlWeek?" · Deload wk":""}</div><div style={{display:"flex",gap:5}}>{items.map(i=>(<div key={i.d.id} title={`${i.d.name} · ${fmtD(i.dt)}`} style={{width:9,height:9,borderRadius:"50%",background:i.st==="done"?D.good:i.st==="today"?D.acc:i.st==="missed"?"#FF4D4D":D.line}}/>))}</div></div>
          {todayItem?(todayItem.done?<div style={{fontSize:13.5,fontWeight:600,color:D.good}}>✓ {todayItem.d.name} done — that's the standard.</div>:<div style={{display:"flex",alignItems:"center",gap:10}}><div style={{flex:1}}><div style={{fontFamily:"'Archivo Black',sans-serif",fontSize:17}}>{todayItem.d.name}</div><div style={{fontSize:10.5,color:D.sub}}>On the schedule today · {todayItem.d.ex.length} exercises{dlWeek?" · lighter deload loads":""}</div></div><button onClick={()=>setRun(todayItem.d.id)} style={{background:D.acc,color:"#0B0B0C",border:0,borderRadius:8,padding:"9px 16px",fontFamily:"'Archivo Black',sans-serif",fontSize:12,cursor:"pointer"}}>START</button></div>):<div style={{fontSize:12.5,color:D.sub}}>🌿 No session scheduled today. Recover with intent — sleep, a walk, mobility, protein.</div>}
        ...
        <div style={{background:`linear-gradient(135deg,${D.acc}22,${D.card})`,border:`1px solid ${D.acc}44`,borderRadius:14,padding:15,display:"flex",alignItems:"center",gap:15}}>
          <Ring pct={lvlPct(xp)} size={84} color={D.acc}><div style={{fontFamily:"'Archivo Black',sans-serif",fontSize:24}}>{lvlOf(xp)}</div><div style={{fontSize:7.5,color:D.sub,letterSpacing:".1em",textTransform:"uppercase"}}>Level</div></Ring>
          <div style={{flex:1}}>
            <div style={{fontSize:10,color:D.acc,fontWeight:700,letterSpacing:".1em",textTransform:"uppercase"}}>{rankOf(xp)}</div>
            ...
            <div style={{fontSize:10.5,color:D.sub,marginTop:2}}>{toNext(xp)} XP to Level {lvlOf(xp)+1}</div>
          </div>
        </div>

        <div onClick={()=>setRecapOpen(true)} style={{background:`linear-gradient(135deg,#3AE0FF22,${D.card})`,border:"1px solid #3AE0FF44",borderRadius:12,padding:13,cursor:"pointer",display:"flex",alignItems:"center",gap:11}}><span style={{fontSize:22}}>📈</span><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13}}>Your Week — Recap</div><div style={{fontSize:10,color:D.sub}}>Sessions, effort, readiness & wins · send it to your coach</div></div><span style={{color:"#3AE0FF"}}>→</span></div>

        <div style={{background:D.card,border:`1px solid ${D.line}`,borderLeft:`3px solid #34D399`,borderRadius:11,padding:13}}>
          <div style={{fontSize:9.5,color:"#34D399",fontWeight:700,letterSpacing:".12em",textTransform:"uppercase",marginBottom:4}}>✝︎ Daily Anchor · {verse[0]}</div>
        ...
        <div style={{background:D.card,border:`1px solid ${D.line}`,borderRadius:11,padding:13,display:"flex",alignItems:"center",gap:14}}>
          ...
          <div style={{flex:1}}>
            ...
            {[["p","Protein","#FF6B2C"],["c","Carbs","#3AE0FF"],["f","Fat","#FFB23A"]].map(([k,l,col])=>(<div key={k} style={{marginBottom:6}}><div style={{display:"flex",justifyContent:"space-between",fontSize:10,marginBottom:2}}><span style={{color:col}}>{l}</span><span className="mono" style={{color:D.sub}}>{Math.round(tot[k])}/{nt[k]}g</span></div><Bar v={tot[k]} t={nt[k]} c={col}/></div>))}
          </div>
        </div>
        ...
        {(()=>{const todayLbl=new Date().toLocaleDateString("en-US",{month:"short",day:"numeric"});const dow=new Date().toLocaleDateString("en-US",{weekday:"long"});const pref=ckday||"Sunday";const checkedToday=checkins.some(c=>c.date===todayLbl);const due=dow===pref&&!checkedToday;const DAYS=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];return(<>
          ...
          <div style={{background:`linear-gradient(135deg,#A78BFA22,${D.card})`,border:"1px solid #A78BFA44",borderRadius:11,padding:13}}>
            <div onClick={()=>setCiOpen(true)} style={{display:"flex",alignItems:"center",gap:11,cursor:"pointer"}}><span style={{fontSize:22}}>📋</span><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13}}>Weekly Check-In</div><div style={{fontSize:10,color:D.sub}}>{checkedToday?"✓ Done today":(checkins.length?`Last: ${checkins[checkins.length-1].date}`:"Not submitted yet")} · +75 XP</div></div><span style={{color:"#A78BFA"}}>→</span></div>
            <div style={{display:"flex",alignItems:"center",gap:7,marginTop:10,paddingTop:10,borderTop:`1px solid ${D.line}`}}><span style={{fontSize:9.5,color:D.sub,letterSpacing:".06em",textTransform:"uppercase",fontWeight:700}}>Reminder day</span><select value={pref} onChange={e=>onSetCkday(e.target.value)} style={{background:D.lift,border:`1px solid ${D.line}`,color:D.ink,borderRadius:6,padding:"4px 7px",fontSize:11,fontFamily:"inherit",outline:"none"}}>{DAYS.map(d=><option key={d} value={d}>{d}</option>)}</select></div>
          </div>
          <div style={{background:D.card,border:`1px solid ${D.line}`,borderRadius:11,padding:13,display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:24}}>🧊</span><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13}}>Streak Freezes · {freezes}</div><div style={{fontSize:10,color:D.sub}}>Earned at every 7-day streak. Spend one to keep a streak alive on a missed day.</div></div><button disabled={freezes<=0} onClick={()=>{if(freezes>0){onUseFreeze();pop("🧊 Freeze used — streak protected");}}} style={{background:freezes>0?"#3AE0FF":D.lift,color:freezes>0?"#0B0B0C":D.sub,border:freezes>0?0:`1px solid ${D.line}`,borderRadius:7,padding:"8px 12px",fontWeight:800,fontSize:11,cursor:freezes>0?"pointer":"default"}}>Use</button></div>
        </>);})()}

        {(()=>{const cm=coachMsgs.find(n=>n.from!=="client");return cm?(<div style={{background:D.card,border:`1px solid ${D.line}`,borderLeft:`3px solid ${client.accent}`,borderRadius:11,padding:13}}><div style={{fontSize:9.5,color:client.accent,fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",marginBottom:4}}>💬 From {cm.author}</div><div style={{fontSize:13,lineHeight:1.5}}>{cm.text}</div><div onClick={()=>setTab("coach")} style={{fontSize:10,color:D.sub,marginTop:7,cursor:"pointer"}}>Open chat →</div></div>):null;})()}
      </div>}

      {tab==="pillars"&&<div style={{paddingTop:6,display:"flex",flexDirection:"column",gap:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontFamily:"'Archivo Black',sans-serif",fontSize:18}}>7 PILLARS</div><div style={{fontSize:10.5,color:D.sub}}>Live long. Live well. — one day at a time.</div></div>
          <div style={{textAlign:"right"}}><div style={{fontFamily:"'Archivo Black',sans-serif",fontSize:24,color:pillDone===7?D.good:D.ink,lineHeight:1}}>{pillDone}<span style={{fontSize:12,color:D.sub}}>/7</span></div><div style={{fontSize:9,color:D.sub,textTransform:"uppercase",letterSpacing:".08em"}}>today</div></div>
        </div>
        <div style={{background:D.card,border:`1px solid ${D.line}`,borderRadius:12,padding:13,display:"flex",flexDirection:"column",alignItems:"center"}}>
          <div style={{fontSize:9.5,color:D.sub,letterSpacing:".12em",textTransform:"uppercase",fontWeight:700,alignSelf:"flex-start"}}>7-Day Balance</div>
        ...
        {PILLARS.map(p=>{const total=p.actions.length;const fullByHab=client.hab&&client.hab[p.id]&&Object.keys(tActs[p.id]||{}).length===0;const acts=tActs[p.id]||{};const done=fullByHab?total:Object.keys(acts).length;const complete=done>=total;const open=openP===p.id;const st=(client.streak&&client.streak[p.id])||0;return(
          <div key={p.id} style={{background:D.card,border:`1px solid ${complete?p.color+"66":D.line}`,borderLeft:`3px solid ${p.color}`,borderRadius:11,overflow:"hidden"}}>
            <div onClick={()=>setOpenP(open?null:p.id)} style={{padding:"12px 13px",display:"flex",alignItems:"center",gap:11,cursor:"pointer"}}>
              ...
              <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,fontSize:14}}>{p.name}{complete?" ✓":""}</div><div style={{fontSize:10.5,color:p.color}}>{p.tag}</div></div>
              <div style={{textAlign:"right"}}><div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:13,color:complete?D.good:D.ink}}>{done}/{total}</div>{st>0&&<div style={{fontSize:9.5,color:D.sub}}>🔥 {st}d</div>}</div>
            ...
            {open&&<div style={{padding:"0 13px 13px"}}>
              ...
              <div style={{display:"flex",gap:8,marginTop:9,padding:"9px 11px",background:p.color+"12",borderRadius:8,borderLeft:`2px solid ${p.color}`}}>
                ...
                <div><div style={{fontSize:9.5,color:p.color,fontWeight:700,letterSpacing:".06em"}}>{p.verse[0]}</div><div style={{fontSize:11.5,color:D.ink,lineHeight:1.45,marginTop:2}}>{p.verse[1]}</div></div>
              </div>
            </div>}
          </div>);})}
        <div style={{textAlign:"center",fontSize:10,color:D.sub,padding:"2px 0 8px"}}>Each action +4 XP · full pillar +15 XP · all 7 = Perfect Day +50</div>
      </div>}

      {tab==="train"&&<div style={{paddingTop:6}}>
        <div style={{fontSize:10,color:D.sub,letterSpacing:".1em",textTransform:"uppercase",fontWeight:700,marginBottom:10}}>Your Program · Week {cw}{dlWeek?" · Deload":""}</div>
      ...
      {tab==="progress"&&<div style={{paddingTop:6,display:"flex",flexDirection:"column",gap:12}}>
        ...
        {stats.vol.length>1&&<div style={{background:D.card,border:`1px solid ${D.line}`,borderRadius:11,padding:13}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:10,color:D.sub,letterSpacing:".1em",textTransform:"uppercase",fontWeight:700}}>Weekly Volume</span><span className="mono" style={{color:D.good,fontSize:11}}>{stats.vol[stats.vol.length-1]}k lb</span></div>
          <Spark data={stats.vol} color="#3AE07A" h={50}/>
        </div>}

        <div style={{background:D.card,border:`1px solid ${D.line}`,borderRadius:11,padding:13}}>
          <div style={{fontSize:10,color:D.sub,letterSpacing:".1em",textTransform:"uppercase",fontWeight:700,marginBottom:11}}>🎖 Badges · {stats.earned.size}/{BADGES.length}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:9}}>{BADGES.map(b=>{const got=stats.earned.has(b.id);return(<div key={b.id} title={b.d} style={{textAlign:"center",opacity:got?1:.32}}><div style={{fontSize:25,filter:got?"none":"grayscale(1)"}}>{b.ic}</div><div style={{fontSize:8.5,color:got?D.ink:D.sub,fontWeight:700,marginTop:3}}>{b.l}</div></div>);})}</div>
        </div>

        <div style={{background:D.card,border:`1px solid ${D.line}`,borderRadius:11,padding:13}}>
          <div style={{fontSize:10,color:D.sub,letterSpacing:".1em",textTransform:"uppercase",fontWeight:700,marginBottom:10}}>🎯 Goals</div>
          {goals.map((g,i)=>{const cur=g.metric==="squat"?(stats.prs["Back Squat"]?.e||0):g.metric==="bench"?(stats.prs["Barbell Bench Press"]?.e||0):g.metric==="deadlift"?(stats.prs["Conventional Deadlift"]?.e||0):g.metric==="bodyweight"?(bodylog.length?bodylog[bodylog.length-1].w:client.bw):g.metric==="sessions"?stats.sessions:null;const tgt=Number(g.target)||0;const pctG=tgt&&cur!=null?Math.min(100,Math.round(cur/tgt*100)):null;const hit=pctG!=null&&cur>=tgt;return(<div key={g.id} style={{marginBottom:11,paddingBottom:11,borderBottom:i<goals.length-1?`1px solid ${D.line}`:"0"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:12.5,fontWeight:600,textDecoration:g.done?"line-through":"none",color:g.done||hit?D.good:D.ink}}>{g.text}{hit&&!g.done?" 🎉":""}</div>{g.due&&<div style={{fontSize:9.5,color:D.sub}}>by {g.due}</div>}</div><div style={{display:"flex",gap:6,alignItems:"center"}}><button onClick={()=>onSaveGoals(goals.map((x,j)=>j===i?{...x,done:!x.done}:x))} style={{background:g.done?D.good:"transparent",border:`1px solid ${g.done?D.good:D.line}`,color:g.done?"#0B0B0C":D.sub,borderRadius:5,width:22,height:22,cursor:"pointer",fontSize:11}}>{g.done?"✓":"○"}</button><button onClick={()=>onSaveGoals(goals.filter((_,j)=>j!==i))} style={{background:"transparent",border:0,color:D.sub,cursor:"pointer",fontSize:13}}>✕</button></div></div>
            {cur!=null&&tgt>0&&<div style={{marginTop:6}}><div style={{display:"flex",justifyContent:"space-between",fontSize:9.5,color:D.sub,marginBottom:3}}><span className="mono">{cur}{g.metric==="sessions"?" sessions":" lb"}</span><span className="mono">target {tgt}</span></div><div style={{height:6,background:D.line,borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:pctG+"%",background:hit?D.good:D.acc,borderRadius:3,transition:"width .4s"}}/></div></div>}
          </div>);})}
      ...
      {tab==="fuel"&&<div style={{paddingTop:6}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div style={{fontSize:10,color:D.sub,letterSpacing:".1em",textTransform:"uppercase",fontWeight:700}}>Today's Macros</div><button onClick={()=>setMealOpen(!mealOpen)} style={{background:D.good,color:"#0B0B0C",border:0,borderRadius:7,padding:"6px 11px",fontWeight:700,fontSize:11,cursor:"pointer"}}>{mealOpen?"Close":"+ Log Meal"}</button></div>
        {mealOpen&&<div style={{background:D.card,border:`1px solid ${D.line}`,borderRadius:9,padding:11,marginBottom:10}}><input value={mf.name} onChange={e=>setMf({...mf,name:e.target.value})} placeholder="Meal name" style={{width:"100%",background:D.lift,border:`1px solid ${D.line}`,borderRadius:6,padding:8,color:D.ink,fontSize:13,outline:"none",marginBottom:7,fontFamily:"inherit"}}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:8}}>{[["p","Protein","#FF6B2C"],["c","Carbs","#3AE0FF"],["f","Fat","#FFB23A"]].map(([k,l,col])=>(<div key={k}><div style={{fontSize:8.5,color:col,fontWeight:700,marginBottom:3,textTransform:"uppercase"}}>{l}</div><input type="number" value={mf[k]} onChange={e=>setMf({...mf,[k]:e.target.value})} placeholder="0" style={{width:"100%",background:D.lift,border:`1px solid ${D.line}`,borderRadius:6,padding:7,color:D.ink,textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:14,outline:"none"}}/></div>))}</div><button onClick={()=>{const p=Number(mf.p)||0,c=Number(mf.c)||0,f=Number(mf.f)||0;if(!p&&!c&&!f)return;onLogMeal({id:"m"+Date.now(),date:today,name:mf.name.trim()||"Meal",p,c,f,kcal:p*4+c*4+f*9});onXP(10);setMf({name:"",p:"",c:"",f:""});setMealOpen(false);pop("🥗 Logged · +10 XP");}} style={{width:"100%",background:D.good,color:"#0B0B0C",border:0,borderRadius:7,padding:10,fontWeight:700,cursor:"pointer"}}>Add Meal</button></div>}
        ...
        {tMeals.length>0&&<div style={{fontSize:10,color:D.sub,letterSpacing:".1em",textTransform:"uppercase",fontWeight:700,margin:"13px 0 6px"}}>Logged Today</div>}
        {tMeals.map(m=>(<div key={m.id} style={{display:"flex",justifyContent:"space-between",background:D.lift,borderRadius:6,padding:"8px 11px",marginBottom:5}}><span style={{fontSize:12}}>{m.name}</span><span className="mono" style={{fontSize:9.5,color:D.sub}}>{m.p}p · {m.c}c · {m.f}f · {m.kcal}kcal</span></div>))}
      </div>}

      {tab==="coach"&&<div style={{paddingTop:6,display:"flex",flexDirection:"column",height:"calc(100vh - 210px)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div style={{fontSize:10,color:D.sub,letterSpacing:".1em",textTransform:"uppercase",fontWeight:700}}>Coach{coachName?` · ${coachName}`:""}</div><button onClick={askAI} disabled={aiBusy} style={{background:"#A78BFA",color:"#0B0B0C",border:0,borderRadius:7,padding:"6px 11px",fontWeight:800,fontSize:10.5,cursor:"pointer",opacity:aiBusy?.6:1}}>{aiBusy?"⏳ Thinking…":"🤖 Ask Coach Adam"}</button></div>
        <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column-reverse",gap:8,marginBottom:10}}>
          {coachMsgs.map((n,i)=>{const mine=n.from==="client";return(<div key={i} style={{alignSelf:mine?"flex-end":"flex-start",maxWidth:"82%",background:mine?client.accent:(n.from==="ai"?"#A78BFA22":D.card),color:mine?"#0B0B0C":D.ink,border:mine?"0":`1px solid ${n.from==="ai"?"#A78BFA55":D.line}`,borderRadius:mine?"12px 12px 3px 12px":"12px 12px 12px 3px",padding:"9px 12px"}}><div style={{fontSize:8.5,opacity:.7,marginBottom:2,fontWeight:700,letterSpacing:".04em",textTransform:"uppercase"}}>{n.author}{n.from==="ai"?" 🤖":""} · {n.date}</div><div style={{fontSize:13,lineHeight:1.45}}>{n.text}</div></div>);})}
    ...
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:D.card,borderTop:`1px solid ${D.line}`,display:"flex",justifyContent:"space-around",padding:"8px 0",zIndex:40}}>{[["home","🏠","Home"],["pillars","🏛","Pillars"],["train","🏋️","Train"],["progress","📈","Progress"],["fuel","🥗","Fuel"],["coach","💬","Coach"]].map(([k,ic,l])=>(<button key={k} onClick={()=>setTab(k)} style={{background:"transparent",border:0,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,opacity:tab===k?1:.45}}><span style={{fontSize:18,filter:tab===k?"none":"grayscale(1)"}}>{ic}</span><span style={{fontSize:7.5,fontWeight:700,color:tab===k?D.acc:D.sub,letterSpacing:".02em",textTransform:"uppercase"}}>{l}</span></button>))}</div>
