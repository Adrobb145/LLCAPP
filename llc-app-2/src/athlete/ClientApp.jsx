  ...
  return(<div style={{minHeight:"100vh",background:D.bg,color:D.ink,fontFamily:"'Inter Tight',system-ui,sans-serif",maxWidth:480,margin:"0 auto",paddingBottom:"calc(86px + env(safe-area-inset-bottom))",position:"relative"}}>
    ...
    <div style={{padding:"0 16px"}}>
      {tab==="home"&&<div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        {(()=>{const ids=(trackedLifts&&trackedLifts.length?trackedLifts:DEFAULT_LIFTS).filter(id=>EXBYID[id]);const byW={};ids.forEach(id=>{Object.keys(clogs).forEach(k=>{const pr=k.split("|");if(pr[2]!==id)return;const wk=+pr[0].slice(1);const e=clogs[k];if(!e||!e.sets)return;let best=0;e.sets.forEach(s=>{if(s.done){const v=e1rm(+s.w||0,+s.r||0);if(v>best)best=v;}});if(best>0){byW[wk]=byW[wk]||{};if(best>(byW[wk][id]||0))byW[wk][id]=best;}});});const weeks=Object.keys(byW).map(Number).sort((a,b)=>a-b);const totals=weeks.map(w=>Math.round(Object.values(byW[w]).reduce((a,b)=>a+b,0)));if(!totals.length)return null;const cur=totals[totals.length-1],first=totals[0],gain=cur-first;const rel=lastBw?Math.round(cur/lastBw*100)/100:0;const W=320,H=90,P=6;const mx=Math.max(...totals),mn=Math.min(...totals)*0.96,rg=(mx-mn)||1;const X=i=>P+(totals.length<=1?(W-2*P)/2:(i/(totals.length-1))*(W-2*P));const Y=v=>P+(1-(v-mn)/rg)*(H-2*P);const linePts=totals.map((v,i)=>X(i)+","+Y(v)).join(" ");const areaPath="M"+X(0)+","+(H-P)+" L"+totals.map((v,i)=>X(i)+","+Y(v)).join(" L")+" L"+X(totals.length-1)+","+(H-P)+" Z";return(<div style={{background:`linear-gradient(135deg,${D.acc}1A,${D.card} 62%)`,border:`1px solid ${D.acc}44`,borderRadius:14,padding:15,overflow:"hidden"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}><div><div style={{fontSize:11,color:D.acc,letterSpacing:".12em",textTransform:"uppercase",fontWeight:800}}>Strength Trajectory</div><div style={{fontFamily:"'Archivo Black',sans-serif",fontSize:28,lineHeight:1.05}}>{cur.toLocaleString()}<span style={{fontSize:13,color:D.sub}}> lb</span></div><div style={{fontSize:11,color:D.sub}}>tracked-lift total &middot; est. 1RM</div></div><div style={{textAlign:"right"}}>{gain>0&&<div style={{display:"inline-block",background:D.good+"22",color:D.good,fontSize:12,fontWeight:800,padding:"3px 8px",borderRadius:20}}>\u25b2 {gain} lb</div>}{rel>0&&<div style={{fontSize:11,color:D.sub,marginTop:6}}>{rel}&times; bodyweight</div>}</div></div>{totals.length>=2?<svg viewBox={"0 0 "+W+" "+H} style={{width:"100%",height:90,display:"block"}} preserveAspectRatio="none"><defs><linearGradient id="stgrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={D.acc} stopOpacity="0.45"/><stop offset="100%" stopColor={D.acc} stopOpacity="0"/></linearGradient></defs><path d={areaPath} fill="url(#stgrad)"/><polyline points={linePts} fill="none" stroke={D.acc} strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>{totals.map((v,i)=><circle key={i} cx={X(i)} cy={Y(v)} r={i===totals.length-1?"3.5":"2"} fill={i===totals.length-1?"#fff":D.acc} stroke={D.acc} strokeWidth="1" vectorEffect="non-scaling-stroke"/>)}</svg>:<div style={{fontSize:12,color:D.sub,fontStyle:"italic",paddingTop:4}}>Log one more week and your strength curve lights up here.</div>}</div>);})()}
